...
class DesktopInterface:
    ...
    def __init__(self, config, transcription_manager=None, config_manager=None):
        ...
        self.transcription_state = (
            "idle"  # 'idle', 'listening', 'transcribing', 'silent'
        )
        ...
        try:
            ...
            for device_id, device_name in devices:
                ...
                if any(
                    ...
                    keyword in name_lower
                    for keyword in ["aggregate", "agregado", "combined", "conjunto"]
                ):
                    ...
                    keyword in name_lower
                    for keyword in ["usb", "wireless", "bluetooth"]
                ):
                    device_info += " [Externo]"
            ...
            logger.info(
                f"Carregados {len(self.available_devices)} dispositivos de áudio (incluindo virtuais)"
            )
    ...
    def _setup_main_tab(self, parent):
        ...
        title_label = ttk.Label(
            title_frame, text="🎙️ Whisper Transcriber", font=("Arial", 16, "bold")
        )
        ...
        self.start_button = ttk.Button(
            controls_frame, text="▶️ Iniciar", command=self.start_transcription
        )
        ...
        self.status_display = ttk.Label(
            controls_frame, textvariable=self.status_var, foreground="red"
        )
        ...
        self.audio_progressbar = ttk.Progressbar(
            controls_frame, variable=self.audio_level_var, maximum=100, length=200
        )
        self.audio_progressbar.grid(
            row=1, column=1, columnspan=2, pady=(10, 0), padx=5, sticky=(tk.W, tk.E)
        )
        ...
        transcriptions_frame = ttk.LabelFrame(
            main_frame, text="Transcrições", padding="10"
        )
        transcriptions_frame.grid(
        ...
        state_label = ttk.Label(
            info_row1, textvariable=self.state_var, font=("Arial", 10, "bold")
        )
        ...
        font_minus_btn = ttk.Button(
            info_row2, text="A-", width=3, command=self._decrease_font
        )
        font_minus_btn.grid(row=0, column=1, padx=(5, 2))

        font_plus_btn = ttk.Button(
            info_row2, text="A+", width=3, command=self._increase_font
        )
        ...
        clear_button = ttk.Button(
            info_row2, text="🗑️ Limpar", command=self.clear_transcriptions
        )
        clear_button.grid(row=0, column=4, padx=5)

        export_button = ttk.Button(
            info_row2, text="💾 Exportar", command=self.export_transcriptions
        )
        export_button.grid(row=0, column=5)

        teleprompter_button = ttk.Button(
            info_row2, text="📺 Teleprompter", command=self.open_teleprompter
        )
    ...
    def _setup_config_tab(self, parent):
        ...
        title_label = ttk.Label(
            config_frame,
            text="⚙️ Configurações de Transcrição",
            font=("Arial", 14, "bold"),
        )
        ...
        ttk.Label(config_frame, text="🎤 Dispositivo de Áudio:").grid(
        ...
        # Modelo Whisper
        ttk.Label(config_frame, text="🧠 Modelo Whisper:").grid(
            row=row, column=0, sticky=tk.W, pady=5
        )
        ...
        # Idioma
        ttk.Label(config_frame, text="🌍 Idioma do Áudio:").grid(
            row=row, column=0, sticky=tk.W, pady=5
        )
        ...
        # Tradução
        ttk.Label(config_frame, text="🔄 Tradução:").grid(
            row=row, column=0, sticky=tk.W, pady=5
        )
        ...
        translation_cb = ttk.Checkbutton(
            translation_frame,
            text="Habilitar tradução",
        ...
        ttk.Label(config_frame, text="🎯 Detecção de Voz:").grid(
            row=row, column=0, sticky=tk.W, pady=5
        )
        ...
        vad_cb = ttk.Checkbutton(
            vad_frame,
            text="Usar detecção por voz (mais rápido que 3s fixo)",
        ...
        ttk.Label(vad_frame, text="Sensibilidade:").grid(
            row=0, column=1, padx=(20, 5), sticky=tk.W
        )
        ...
        self.vad_aggressiveness_combo = ttk.Combobox(
            vad_frame,
            textvariable=self.vad_aggressiveness_var,
            values=vad_aggr_values,
            state="readonly",
            width=15,
        )
        ...
        tooltip_text = (
            "Conservador: Só detecta fala muito clara\n"
        ...
        ttk.Label(config_frame, text="🎯 Traduzir para:").grid(
            row=row, column=0, sticky=tk.W, pady=5
        )
        ...
        self.target_language_combo.grid(
        ...
        try:
            self.target_language_combo.current(
                target_language_values.index(target_display)
            )
        except ValueError:
        ...
        self.current_config_label = ttk.Label(
            config_frame,
            textvariable=self.current_config_var,
            font=("Arial", 9),
            foreground="gray",
        )
    ...
    def export_transcriptions(self):
        ...
        try:
            ...
            if filename:
                ...
                header += (
                    f"Exportado em: {datetime.now().strftime('%d/%m/%Y às %H:%M:%S')}\n"
                )
                ...
                messagebox.showinfo(
                    "Sucesso", f"Transcrições exportadas para:\n{filename}"
                )
    ...
    def _update_current_config_display(self):
        ...
        try:
            ...
            config_text = (
                f"Atual: Modelo={model} | Dispositivo={device} | Idioma={lang_display}"
            )
    ...
    def _update_ui_elements(self):
        ...
        try:
            ...
            stats_text = (
                f"Total: {self.current_stats['total_transcriptions']} transcrições"
            )
            if self.current_stats["translation_count"] > 0:
            ...
            if self.is_running and self.start_time:
                self.current_stats["uptime_seconds"] = int(
        ...
        def update_ui():
            ...
            for i in range(len(lines) - 1, -1, -1):
                if lines[i].startswith("[") and "]" in lines[i]:
                    ...
                    insert_pos = f"{i+2}.0"
                    self.transcriptions_text.insert(
                        insert_pos, f"    → {translation}\n"
                    )
    ...
    def on_closing(self):
        ...
        if self.is_running:
            if messagebox.askokcancel(
                "Sair", "Transcrição está ativa. Deseja parar e sair?"
            ):
                self.stop_transcription()
...
class TeleprompterWindow:
    ...
    def _create_window(self):
        ...
        self.root.wm_attributes(
            "-alpha", 0.95
        )  # Ligeiramente transparente, mas controles visíveis
    ...
    def _update_transparency(self):
        ...
        try:
            ...
            alpha_value = max(
                0.7, self.transparency
            )  # Mínimo de 70% para manter controles visíveis
    ...
    def _setup_ui(self):
        ...
        # Tamanho da fonte
        tk.Label(
            config_frame, text="Fonte:", bg=self.current_bg, fg=self.text_color
        ).pack(side="left")

        font_down_btn = tk.Button(
            config_frame, text="A-", command=self._decrease_font, width=3
        )
        font_down_btn.pack(side="left", padx=2)

        font_up_btn = tk.Button(
            config_frame, text="A+", command=self._increase_font, width=3
        )
        font_up_btn.pack(side="left", padx=2)

        # Transparência
        tk.Label(
            config_frame, text="Opacidade:", bg=self.current_bg, fg=self.text_color
        ).pack(side="left", padx=(10, 0))

        opacity_down_btn = tk.Button(
            config_frame, text="-", command=self._decrease_opacity, width=3
        )
        opacity_down_btn.pack(side="left", padx=2)

        opacity_up_btn = tk.Button(
            config_frame, text="+", command=self._increase_opacity, width=3
        )
        opacity_up_btn.pack(side="left", padx=2)

        # Cores
        color_btn = tk.Button(
            config_frame, text="🎨 Cores", command=self._change_colors
        )
        ...
        toggle_gui_btn = tk.Button(
            config_frame, text="🔄 GUI", command=self.parent.toggle_gui_visibility
        )
        ...
        self.root.bind(
            "<F2>", lambda e: self.parent.toggle_gui_visibility()
        )  # F2 para alternar GUI
    ...
    def _change_colors(self):
        ...
        bg_color = colorchooser.askcolor(
            title="Escolher cor de fundo", initialcolor=self.bg_color
        )
        ...
        if bg_color[1]:  # Se cor foi escolhida
            ...
            text_color = colorchooser.askcolor(
                title="Escolher cor do texto", initialcolor=self.text_color
            )
    ...
    def _apply_colors(self):
        ...
        if hasattr(self, "text_widget"):
            self.text_widget.config(
                bg=self.current_bg, fg=self.text_color, insertbackground=self.text_color
            )
